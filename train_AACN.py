...
def run_training(ds, lr, epochs, output, num_heads, k, v, img_size, name):      
    print("Setup model..")
    model = resnet50(num_classes=CLASSES, attention=[False,True,True,True], num_heads=num_heads, k=k, v=v, image_size=img_size)
    ...
    learn = Learner(ds, model, opt_func=Adam, metrics=accuracy, 
                    cbs=[
                        CSVLogger(fname = './logs/attention/imagenette{}_AACN_{}.csv'.format(TIME, name), append=True),
                        SaveModelCallback(monitor='accuracy', fname= output + 'imagenette{}_AACN_{}'.format(TIME, name))
                    ]
                   )
...
def main():
    ...
    run_training(ds, args.lr, args.epochs, args.output, args.num_heads, args.k, args.v, args.img_size, args.name)
